    ...
    def _check_input_types(self, data):
        ...
        for key, value in data.items():
            if not isinstance(key, str):
                raise TypeError('keys of `data` must be a string')
            if not isinstance(value, np.ndarray):
                raise TypeError('values of `data` must be a numpy array')
            if value.ndim != 1:
                raise ValueError('values must be a one-dimensional numpy array')

    def _check_array_lengths(self, data):
        #check if all the values have the same length
        for i, value in enumerate(data.values()):
            if i==0:
                length = len(value)
            elif length != len(value):
                raise ValueError('All values of data must be of same length')
